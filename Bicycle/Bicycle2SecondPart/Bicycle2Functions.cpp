...
void SetSpeed() // new SetSpeed for lab 5
{
	...
	while (true)
	{
		...
		{
			cout << "\nRetry too many times. Speed not set, good bye." << endl;
			return;
		}
		else
		{		
...
void GetSelection()
{
	...
	while (true) // continue to get user input until user input falls within correct range.
	{

		static int n = { 5 }; // retry n times, then exit program

		if (--n < 0) // retry checking.
		{
			cout << "\nRetires too many times, good bye." << endl;
			return;
		}
		else
		{
